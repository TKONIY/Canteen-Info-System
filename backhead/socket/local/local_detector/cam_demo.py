...
def arg_parse():
    ...
    parser.add_argument("--host", dest='host', help="IP of the server to connected",
...
if __name__ == '__main__':
    ...
    开启服务器
    """
    print("等待socket连接..."+args.host)
    s = socket.socket()
    s.connect((args.host, 9999))
    print("socket已连接")
...
#            im_dim = im_dim.repeat(output.size(0), 1)
            ...
            """
            # print([classes[int(x[-1])] for x in output])
            num_person = (output[:, -1] == 0).sum().item()
            request = recvBytes(csocket=s, length=4)
            request = request.decode('utf-8')
            print("当前人数为" + str(num_person))

            if request == "next":
                s.send(str(num_person).ljust(16).encode('utf=8'))
            else:
                continue
